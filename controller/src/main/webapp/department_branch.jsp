...
<c:forEach var="branch" items="${listOfAllBranchDto}" varStatus="status">
            ...
            <tr>
                ...
                <td>
                    <form name="deleteFromBranch" method="post" action="department_branch">
                        ...
                        <button type="submit" name="action" value="deleteFromBranch">Удалить</button>
                    </form>
                </td>
            </tr>
        </c:forEach>
    ...
    <br>
    <c:url value="/department-form.jsp" var="depAddUrl">
        <c:param name="idBranch" value="${branch.id}"/>
        <c:param name="action" value="addInBranch"/>
    </c:url>
    <button class="btn success"><a href="${depAddUrl}">Добавить для филиала</a></button>
</c:forEach>
